...
export const commandAPI = {
  ...
  createCommand: async (commandData) => {
    try {
      ...
      const backendData = {
        fullname: commandData.name,
        number: commandData.phone,
        flor: commandData.floor || commandData.isGroundOrFirst || '',
  ...
  getCommandsDirect: async () => {
    try {
      console.log('🌐 Making direct request to: https://gokrixo.onrender.com/GetCommands');
      const response = await axios.get('https://gokrixo.onrender.com/GetCommands', {
        timeout: 10000,
        headers: {
          'Content-Type': 'application/json',
      ...
      console.log('✅ Direct commands request successful:', response.data);
      return response.data;
    } catch (error) {
      console.error('❌ Direct commands request failed:', error);
...
export const workerAPI = {
  ...
  getWorkersDirect: async () => {
    try {
      console.log('🌐 Making direct request to: https://gokrixo.onrender.com/GetWorkers');
      const response = await axios.get('https://gokrixo.onrender.com/GetWorkers', {
        timeout: 10000,
        headers: {
          'Content-Type': 'application/json',
        }
      });
      console.log('✅ Direct workers request successful:', response.data);
      return response.data;
    } catch (error) {
      console.error('❌ Direct workers request failed:', error);
